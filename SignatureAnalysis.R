signatureAnalysis_GDC <- function(tissueID, geneName){
  ...
  row.names(expsNeeded) <- expsNeeded$geneid
  ...
  vioPlot <- ggplot(plotTable, aes(x = Type, y = Means, color = Type, fill = Type))  + 
    geom_violin(lwd = 1) + 
    geom_boxplot(lwd = 1.2, outlier.size = -1,  width = 0.1, color = if (length(unique(plotTable$Type)) == 2) {twoCol} else if(length(unique(plotTable$Type)) == 3){threeCol}) +
