...
multipleGeneAnalysis <- function(tissueID, geneName) {
  clinTable <- read_xlsx(path = "GDC_clinical.xlsx", sheet = 1)
  ...
  geneNameToUse <- geneName[geneName %in% expTable$geneid]
  
  expsNeeded <- expTable[expTable$geneid %in% geneName,]
  expsNeeded <- as.data.frame(t(expsNeeded))
  colnames(expsNeeded) <- expsNeeded[1,]
  expsNeeded <- expsNeeded[-1,]
  expsNeeded <- expsNeeded[,rev(geneNameToUse)]
  ...
  tableForPlot$Gene <- factor(x = tableForPlot$Gene, levels = unique(tableForPlot$Gene))
...
targetgram_GDC <- function(tissueID, geneName,logtrans){
  clinTable <- read_xlsx(path = "GDC_clinical.xlsx", sheet = 1)
  TCGAID <- unlist(strsplit(tissueID[[1]], split = ".", fixed = T))[2]
  clinTable <- clinTable[clinTable$`Project ID` == TCGAID,]
